...
const (
	createPendingUserQuery = `INSERT INTO users (user_id, first_name, last_name, contact, phone, email, password, date_birthday, gender, role)
								...
								RETURNING user_id, first_name, last_name, contact, phone, email, password, date_birthday, gender, role, created_at, updated_at`

	findByContact = `SELECT user_id FROM users WHERE contact = $1`
